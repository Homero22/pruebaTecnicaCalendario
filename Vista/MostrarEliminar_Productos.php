<?php
include("../Config/conexion.php");
$sql="SELECT * from productos";
$resultado= mysqli_query($conexion,$sql);
include("../Vista/TablaProductos.php");
while($mostrar=mysqli_fetch_array($resultado)){
    ?>
    
     <table  border="2">
     <tbody>
    <tr>
   
        <td width ="116"> <?php echo $mostrar['Código']?>       </td>
        <td width ="105"> <?php echo $mostrar['Nombre']?>   </td>
        <td width ="105"> <?php echo $mostrar['Cod_categoria']?> </td>
        <td width ="70"> <?php echo $mostrar['Cantidad']?>    </td>
        <td width ="125"> <?php echo $mostrar['Descripción']?>    </td>
        <td width ="60"> <?php echo $mostrar['Precio']?>    </td>

        <script src="../Modelo/Clientes.js" ></script>
        <td width ="60"> <a href="../Modelo/EditarProductos.php?codigo=<?php echo $mostrar['Código']?>">Editar</a> </td>
        <td width ="75"> <a href="../Modelo/EliminarProductos.php?codigo=<?php echo $mostrar['Código']?>" onclick="return Confirmar()" >Eliminar</a> </td>
    </tr>
    </tbody>
    </table>
    <link rel="stylesheet" href="../publica/css/clientes.css">
<nav class="main-menu">
            <ul>
                <li>
                    <a href="../Controlador/controladorProductos.php?var1=1">
                        <img src="../publica/img/registro-en-linea.png" alt="" id="imm">
                        <i class="fa fa-home fa-2x"></i>
                        <span class="nav-text">
                            Ingreso de productos
                        </span>
                    </a>
                  
                </li>
                <li class="has-subnav">
                    <a href="../Controlador/controladorProductos.php?var1=2">
                    <img src="../publica/img/buscar_productos.png" alt="" id="imm">
                        <i class="fa fa-laptop fa-2x"></i>
                        <span class="nav-text">
                            Buscar productos
                        </span>
                    </a>
                    
                </li>
                <li class="has-subnav">
                    <a href="../Controlador/controladorProductos.php?var1=3">
                    <img src="../publica/img/editar.png" alt="" id="imm">
                       <i class="fa fa-list fa-2x"></i>
                        <span class="nav-text">
                            Editar / Eliminar
                        </span>
                    </a>
                    
                </li>
                <li class="has-subnav">
                    <a href="../Controlador/controladorProductos.php?var1=4">
                    <img src="../publica/img/ojo.png" alt="" id="imm">
                       <i class="fa fa-folder-open fa-2x"></i>
                        <span class="nav-text">
                            Mostrar productos
                        </span>
                    </a>
                   
                </li>
                <li>
                    <a href="../index.html">
                        <img src="../publica/img/menuPrincipal.jpg" alt="" id="imm">
                        <i class="fa fa-home fa-2x"></i>
                        <span class="nav-text" >
                          Menú Principal
                        </span>
                    </a>
                </li>
    </nav>
   
<?php
}
?>

